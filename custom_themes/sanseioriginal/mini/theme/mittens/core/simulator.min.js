...
$$CookieWriter.$inject=['$document','$log','$browser'];angular.module('ngCookies').provider('$$cookieWriter',function $$CookieWriterProvider(){this.$get=$$CookieWriter;});})(window,window.angular);(function(){var is_exports,root,supLocales;is_exports=typeof exports!=="undefined"&&exports!==null;root=is_exports?exports:this;if(!root.sup){root.sup={};}
...
console.info('--------------------------------------');};}).call(this);(function(){angular.module('mittens',['ngSanitize','supLocales','supAngularWysiwyg']);angular.module('mittens').constant('Project',{name:'mittens',version:'1.1.1',creator:['Redyyu'],rip:['[Dtynn]','[Flingjie]','[Windfarer]','[Majik]']}).config(['$qProvider','$locationProvider',function($qProvider,$locationProvider){$qProvider.errorOnUnhandledRejections(false);return $locationProvider.hashPrefix('');}]);}).call(this);(function(){var indexOf=[].indexOf;angular.module('mittens').directive('editorCanvas',['$compile',function($compile){return{restrict:'AE',scope:{},controller:['$scope','$rootScope','$element','$compile','$interval','$http','supLocales',function($scope,$rootScope,$element,$compile,$interval,$http,supLocales){var init;$scope._=angular.translate;$rootScope.$watch('station',function(station){if(station){return init(station);}});return init=function(station){var check_keymap,compile_tpl,content_type,curr_editor,e,editor_metas,keymap,strict_mode,strict_template_filter;curr_editor=station.editor_context;strict_mode=station.strict_mode;if(strict_mode){console.info('Canvas running in Strict mode.');}
$scope.g={};$scope.query={};editor_metas=curr_editor.metas;$scope.base_url=$rootScope.base_url=editor_metas.base_url;$scope.theme_url=$rootScope.theme_url=editor_metas.theme_url;$scope.lib_url=$rootScope.lib_url=editor_metas.lib_url;$scope.locale=$rootScope.locale=editor_metas.locale;$scope.lang=$rootScope.lang=editor_metas.lang;$scope.site_meta=editor_metas.site_meta;$scope.theme_meta=editor_metas.theme_meta;$scope.menu=editor_metas.menu;$scope.languages=editor_metas.languages;$scope.socials=editor_metas.socials;$scope.slot=editor_metas.slot;content_type=editor_metas.content_type[station.curr_content_type];$scope.content_type=content_type;$scope.visit=curr_editor.visit;compile_tpl=function(theme_dir,template){return $http.get(theme_dir+'/'+template).then(function(data){var compile,e;try{supLocales.load(editor_metas.locale,station.localized_dict);}catch(error){e=error;console.error('Canvas localize error:',e);}
$element.html(data.data);if(strict_mode){strict_template_filter($element[0]);}
try{compile=$compile($element.contents());compile($scope);}catch(error){e=error;console.error('Canvas rendering error:',e);}
$scope.g={};return $scope.query={};});};strict_template_filter=function(el){var saw_editors;saw_editors=el.querySelector('[sup-angular-wysiwyg]');if(saw_editors){saw_editors.setAttribute('readonly',true);}
return el;};try{compile_tpl(station.theme_dir,station.template+'.tpl');}catch(error){e=error;console.log(e);}
$scope.meta=station.file.meta;$scope.content=station.file.content;if(station.strict_mode){return $rootScope.escape=true;}else{keymap={};check_keymap=function(codes){var code,i,len;if(typeof codes==='number'){codes=[codes];}
for(i=0,len=codes.length;i<len;i++){code=codes[i];if(!keymap[code]){return false;}}
return true;};return $element.on('keydown keyup',function(e){var _key_down,escape,key_codes,ref;key_codes=[17,27];_key_down=e.type==='keydown';if((ref=e.keyCode,indexOf.call(key_codes,ref)<0)||keymap[e.keyCode]===_key_down){return;}
keymap[e.keyCode]=_key_down;if(check_keymap(key_codes)){escape=!$rootScope.escape;$rootScope.escape=escape;return console.info('Editor: escape '+escape);}});}};}]};}
]).directive('supQuery',['$rootScope','Loader',function($rootScope,Loader){return{scope:{attrs:'=',contentType:'=',sortby:'=',perpage:'=',paged:'=',term:'=',withContent:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var content_type,curr_editor,e,perpage,queries,query_limit,station,term,theme_opts,with_content;if(!ngModel){return;}
...
station=$rootScope.station;if(!station){return;}
try{curr_editor=station.editor_context||{};theme_opts=curr_editor.metas.theme_meta.options||{};}catch(error){e=error;console.error(e);theme_opts={};}
content_type=scope.contentType||attrs.supQuery;with_content=angular.bool(scope.withContent);perpage=angular.parse_int(scope.perpage)||theme_opts.perpage||12;term=scope.term;return Loader.query_files(content_type,perpage,term,with_content).then(function(files){return ngModel.$setViewValue(files);}).catch(function(e){return console.log(e);});}};}
]).directive('supCategorize',['$rootScope','Loader',function($rootScope,Loader){return{scope:{termKeys:'='},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var key,queries,query_limit,station,term_keys;if(!ngModel){return;}
...
return Loader.get_category().then(function(category){return ngModel.$setViewValue(category);});}};}
...
]).directive('supBindHtml',function(){return{restrict:'A',compile:function(){return function(scope,element,attrs){return scope.$watch(attrs.supBindHtml,function(val){return element.html(val||'');});};}};});}).call(this);(function(){angular.module('mittens').config(['$sceDelegateProvider',function($sceDelegateProvider){return $sceDelegateProvider.resourceUrlWhitelist(['self']);}]);}).call(this);(function(){var indexOf=[].indexOf;angular.module('mittens').filter("trust",['$sce',function($sce){return function(str){return $sce.trustAsHtml(str);};}]).filter("url",['$rootScope',function($rootScope){return function(url,remove_args,remove_hash){var index;if(typeof url!=='string'){return url;}
...
return output;};}).filter("thumbnail",function(){return function(pic_url,preset_name){var e,engine_str,params,preset_map,suffix,url_pairs;if(!angular.isUrl(pic_url)){return pic_url;}
...
return is_empty;};}).filter("price",function(){return function(val,use_currency,fractionSize){return angular.price(val,use_currency,fractionSize);};});}).call(this);(function(){angular.module('mittens').constant('config',{LIB_URL:SIM_CONF.lib_url||'',THEMES_DIR:'themes',THEME_NAME:SIM_CONF.theme_name||'default',THEME_CONFIG_FILE:'config.json',CONTENT_DIR:'content',SITE_DATA_FILE:'site.json',VISIT_DATA_FILE:'visit.json',FILES_DATA_FILE:'contents.json',LANGUAGES_DIR:'languages',TRANSLATE_EXT:'.lang'});}).call(this);(function(){var indexOf=[].indexOf;angular.module('mittens').run(['$rootScope','$http','config','Project','Loader',function($rootScope,$http,config,Project,Loader){var base_url,curr_file_slug,curr_tempalte_slug,lang_dir,lib_url,params,theme_dir,theme_url;console.info("-----------------------------");console.info(Project.name+":",Project.version);console.info("Creators:",Project.creator.join(', '));console.info("R.I.P:",Project.rip.join(', '));console.info("-----------------------------");params=angular.getURLParams(location.search);curr_tempalte_slug=params.tpl||'index';curr_file_slug=params.file||'page';base_url=angular.strip(location.protocol+'//'+location.host+location.pathname,'/');theme_url=base_url+'/'+config.THEMES_DIR+'/'+config.THEME_NAME;lib_url=config.LIB_URL;theme_dir=config.THEMES_DIR+'/'+config.THEME_NAME;lang_dir=theme_dir+'/'+config.LANGUAGES_DIR;$rootScope.station={template:curr_tempalte_slug,strict_mode:SIM_CONF.strict_mode||false,theme_dir:theme_dir,editor_context:{metas:{theme_url:theme_url,base_url:base_url,lib_url:lib_url,theme_meta:{}}}};return $http.get(theme_dir+'/'+config.THEME_CONFIG_FILE).then(function(result){return $rootScope.station.editor_context.metas.theme_meta=result.data;}).then(function(){return $http.get(config.CONTENT_DIR+'/'+config.SITE_DATA_FILE);}).then(function(data){var lang,locale,site;console.info(data.data);site=data.data;locale=site.locale||'en_US';lang=locale.split('_')[0];angular.update_dict($rootScope.station.editor_context.metas,{locale:locale,lang:lang,languages:angular.pop(site.meta,'languages'),menu:site.menus,slot:site.slots,site_meta:site.meta,content_type:site.content_types||{'page':'Pages'}});return Loader.get_localized(locale,lang_dir).then(function(localized){return $rootScope.station.localized_dict=localized;});}).then(function(){return Loader.get_visit_data().then(function(visit){return $rootScope.station.editor_context.visit=visit;});}).then(function(){return Loader.get_page_data(curr_file_slug).then(function(file){$rootScope.station.file=file;return $rootScope.station.curr_content_type=file.content_type;});}).catch(function(error){return console.error(error);});}]).service('Loader',['$http','$q','config',function($http,$q,config){var self;self=this;this.get_localized=function(locale,lang_dir){var deferred,lang,lang_file,locale_file;locale=(locale||'en_US').toLowerCase();lang=locale.split('_')[0];locale_file=lang_dir+'/'+locale+config.TRANSLATE_EXT;lang_file=lang_dir+'/'+lang+config.TRANSLATE_EXT;deferred=$q.defer();$http.get(lang_file).then(function(result){return deferred.resolve(result.data);}).catch(function(error){return $http.get(locale_file.toLowerCase()).then(function(result){return deferred.resolve(result.data);}).catch(function(error){console.error(error);return deferred.resolve([]);});});return deferred.promise;};this.get_visit_data=function(){var deferred;deferred=$q.defer();$http.get(config.CONTENT_DIR+'/'+config.VISIT_DATA_FILE).then(function(result){return deferred.resolve({ip:result.data.ip,page:result.data.page,pv:result.data.pv,uv:result.data.uv,vs:result.data.vs});}).catch(function(error){console.error(error);return deferred.resolve({ip:0,page:0,pv:0,uv:0,vs:0});});return deferred.promise;};this.get_page_data=function(file_slug){var deferred;if(!file_slug){file_slug='page';}
deferred=$q.defer();$http.get(config.CONTENT_DIR+'/'+file_slug+'.json').then(function(result){var content,file_data,k,meta,v;file_data=result.data;content=angular.pop(file_data,'content');meta=angular.pop(file_data,'meta');for(k in file_data){v=file_data[k];meta[k]=v;}
meta.updated=angular.now();meta.creation=angular.now();console.log(meta);return deferred.resolve({meta:meta,content:content});}).catch(function(error){console.error(error);return deferred.resolve({});});return deferred.promise;};this.get_category=function(term_keys){var deferred;deferred=$q.defer();$http.get(config.CONTENT_DIR+'/'+config.SITE_DATA_FILE).then(function(result){var category,term,terms;category=result.data.categories;terms=category.terms;if(angular.isArray(term_keys)){terms=(function(){var i,len,ref,results;results=[];for(i=0,len=terms.length;i<len;i++){term=terms[i];if(ref=term.key,indexOf.call(term_keys,ref)>=0){results.push(term);}}
return results;})();}
return deferred.resolve({name:category.name,content_types:category.content_types,terms:category.terms});}).catch(function(error){console.error(error);return deferred.resolve(null);});return deferred.promise;};this.get_segments=function(content_type,parent,limit,tmpls){var deferred;if(content_type===void 0){content_type='page';}
if(parent===void 0){parent='index';}
if(limit===void 0){limit=24;}
deferred=$q.defer();$http.get(config.CONTENT_DIR+'/'+config.FILES_DATA_FILE).then(function(result){var _file,contents,f,i,k,len,ref,ref1,v;contents=[];ref=result.data.contents;for(i=0,len=ref.length;i<len;i++){f=ref[i];if(f.parent===parent&&f.content_type===content_type&&(ref1=f.template,indexOf.call(tmpls,ref1)>=0)){_file=f.meta;for(k in f){v=f[k];if(k!=='meta'){_file[k]=v;}}
contents.push(_file);}}
if(contents.length>limit){contents.length=limit;}
return deferred.resolve(contents);}).catch(function(error){console.error(error);return deferred.resolve([]);});return deferred.promise;};this.query_files=function(content_type,perpage,term,with_content){var deferred,parent;if(content_type===void 0){content_type='page';}
if(parent===void 0){parent='index';}
if(perpage===void 0){perpage=12;}
deferred=$q.defer();$http.get(config.CONTENT_DIR+'/'+config.FILES_DATA_FILE).then(function(result){var _file,contents,f,i,k,len,params,ref,v;contents=[];params=result.data.params;ref=result.data.contents;for(i=0,len=ref.length;i<len;i++){f=ref[i];if(f.content_type===content_type){if(term&&indexOf.call(f.terms,term)<0){continue;}
_file=f.meta;for(k in f){v=f[k];if(k==='meta'){continue;}
if(!with_content&&k==='content'){continue;}
_file[k]=v;}
contents.push(_file);}}
if(contents.length>perpage){contents.length=perpage;}
return deferred.resolve({contents:contents,perpage:perpage,paged:params.paged,total_pages:params.total_pages||0,total_count:params.total_count||0,page_range:params.page_range||[],has_prev:params.has_prev,has_next:params.has_next});}).catch(function(error){console.error(error);return deferred.resolve([]);});return deferred.promise;};return this;}]);}).call(this);(function(){angular.module('mittens').directive('supWidgetCarousel',['$rootScope',function($rootScope){return{scope:{default:'@',allowTypes:'@',limit:'@'},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var _default,clip,default_data,e,i,len,station;if(!ngModel){return;}
station=$rootScope.station;if(!station){return;}
...
return ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){return ngModel.$setViewValue(default_data);}};}};}]);}).call(this);(function(){angular.module('mittens').directive('supWidgetLines',['$rootScope',function($rootScope){return{scope:{default:"@",limit:"@"},restrict:'AE',require:'?ngModel',link:function(scope,element,attrs,ngModel){var _default,default_data,e,i,len,station,str;if(!ngModel){return;}
station=$rootScope.station;if(!station){return;}
...
if(!angular.isArray(default_data)){default_data=[];}
return ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){return ngModel.$setViewValue(default_data);}};}};}]);}).call(this);(function(){angular.module('mittens').directive('supWidgetSegments',['$rootScope','Loader',function($rootScope,Loader){return{scope:{content_type:'@',parent:'@',limit:'@'},restrict:'AE',require:'ngModel',link:function(scope,element,attrs,ngModel){var curr_editor,e,file,queries,query_limit,station,theme_meta,tmpl,tmpls,update_add_element;if(!ngModel){return;}
...
station=$rootScope.station;if(!station){return;}
try{curr_editor=station.editor_context||{};theme_meta=curr_editor.metas.theme_meta({});}catch(error){e=error;console.error(e);theme_meta={};}
if(!angular.isArray(theme_meta.templates)){theme_meta.templates=[];}
tmpls=(function(){var i,len,ref,results;ref=theme_meta.templates;results=[];for(i=0,len=ref.length;i<len;i++){tmpl=ref[i];if(angular.startswith(tmpl,'^')){results.push(angular.strip(tmpl,'^'));}}
return results;})();file=station.file;scope.limit=Math.min(angular.parse_int(scope.limit,24,true),24);update_add_element=function(data){var el_add;el_add=angular.element(element[0].querySelector('[segment-create]'));if(!data||!el_add){return;}
if(data.length>=scope.limit){return el_add.addClass('ng-hide');}else{return el_add.removeClass('ng-hide');}};return Loader.get_segments(content_type,parent,limit,tmpls).then(function(segments){update_add_element(segments);return ngModel.$setViewValue(segments);});}};}]);}).call(this);(function(){angular.module('mittens').directive('supWidgetSeries',['$rootScope',function($rootScope){return{scope:{default:'@',allowTypes:'@',limit:'@'},restrict:'AE',require:'ngModel',link:function(scope,element,attrs,ngModel){var _default,check_limit,default_data,e,i,item,len,station;station=$rootScope.station;if(!station){return;}
try{_default=scope.$eval(scope.default);default_data=[];for(i=0,len=_default.length;i<len;i++){item=_default[i];default_data.push({src:item.src||'',type:item.type||'image',title:item.title||'',caption:item.caption||'',link:'',target:'',class:'',poster:item.poster||'',misc:item.misc||'',tags:item.tags||[]});}}catch(error){e=error;default_data=[];}
if(!angular.isArray(default_data)){default_data=[];}
scope.limit=Math.min(angular.parse_int(scope.limit,24,true),24);check_limit=function(data){var el,el_add;el=element[0].querySelector('[series-item-create]');if(!data||!el){return;}
el_add=angular.element(el);if(data.length>=scope.limit){el_add.addClass('ng-hide');if(data.length>scope.limit){return data.length=scope.limit;}}else{return el_add.removeClass('ng-hide');}};return ngModel.$render=function(){if(!angular.isArray(ngModel.$viewValue)){ngModel.$setViewValue(default_data);}
return check_limit(ngModel.$viewValue);};}};}]);}).call(this);(function(){angular.module('mittens').directive("supWidgetText",['$rootScope','$timeout','$interpolate',function($rootScope,$timeout,$interpolate){return{scope:{default:"@"},restrict:"AE",require:"?ngModel",link:function(scope,element,attrs,ngModel){var clear_html,default_data,dirty,readViewText,station;if(!ngModel){return;}
station=$rootScope.station;if(!station){return;}
...
ngModel.$render=function(){var timer;if(!ngModel.$viewValue&&!dirty){element.html(default_data);timer=$timeout(function(){readViewText();return $timeout.cancel(timer);},0);}else{element.html(ngModel.$viewValue||"");}
return dirty=true;};clear_html=function(html){html=html.replace(/<div\s*.*?>/ig,'<br>');html=html.replace(/<(?!br\s*\/?)[^>]+>/ig,'');if(html==="<br>"){html="";}
return html;};return readViewText=function(){var html;html=clear_html(element.html());if(html==="<br>"){html="";}
return ngModel.$setViewValue(html.trim());};}};}]);}).call(this);