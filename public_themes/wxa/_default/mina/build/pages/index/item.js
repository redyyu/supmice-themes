...
(function() {
  ...
  core.Page({
    ...
    onShareAppMessage: function() {
      var e, img_src, meta, self, share_obj;
      self = this;
      meta = self.data.meta;
      if (meta) {
        try {
    ...
    select_spec: function(e) {
      var curr_opt_idx, curr_spec, i, j, len, len1, opt, ref, self, shelf_spec, spec, spec_key;
      self = this;
    ...
    buy: function(e) {
      ...
      cart_item = {
        ...
        price: sku.dis_price >= 0 ? sku.dis_price : sku.price,
      ...
      if (action_type === 'purchase') {
        ...
        if (old_cart_item && old_cart_item.signature === cart_item.signature) {
          cart_item.amount += old_cart_item.amount;
          return app.cart.update(cart_item);
        } else {
          return app.cart.add(cart_item);
        }
      }
    },
    // helpers
    ...
    _organize_spec: function(shelf_spec, sku_list) {
      var __check_opt_in_skus, _tmp_list, i, j, l, len, len1, len2, len3, m, op, opt, ref, ref1, s, sel_op, self, sku, spec, stock_list;
      self = this;
        ...
        for (j = 0, len1 = shelf_spec.length; j < len1; j++) {
          ...
          if (sel_op.value) {
            _tmp_list = (function() {
              var len3, m, results;
              results = [];
              for (m = 0, len3 = _tmp_list.length; m < len3; m++) {
                sku = _tmp_list[m];
                if (sku.attrs[s.key] === sel_op.value && sku.qty) {
                  results.push(sku);
                }
              }
              return results;
    ...
    _find_selected_sku: function() {
      ...
      if (!self.data.shelf.is_various) {
        if (self.data.sku_list[0] && self.data.sku_list[0].qty) {
    ...
    _get_voucher_names: function() {
      var e, self, voucher;
      self = this;
      try {
        return (function() {
          var i, len, ref, results;
          ref = self.data.discount.vouchers;
          results = [];
          for (i = 0, len = ref.length; i < len; i++) {
            voucher = ref[i];
            results.push(voucher.tagname);
          }
          return results;
        })();
      } catch (error) {
